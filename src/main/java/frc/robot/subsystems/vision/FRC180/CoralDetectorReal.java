...
public class CoralDetectorReal implements CoralDetector {
  ...
  private static final double CLOSE_CORAL_DISTANCE = 0.6;
  private static final double CLOSE_CORAL_TX = 10;
  ...
  private static final double SIMILAR_CORAL_THRESHOLD = 0.75;
    ...
    for (RawDetection detection : sortedDetections) {
      ...
      if (auto) {
        ...
        for (RawDetection algae : algaeDetections) {
          if (Math.abs(degrees - algae.txnc)
              < ALGAE_AVOID_THRESHOLD_DEGREES) { // && detection.tync < algae.tync) {
            rejectionAlgae = true;
            break;
          }
        }
