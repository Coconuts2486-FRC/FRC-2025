...
public class CoralDetectorSim implements CoralDetector {
   ...
   *
   * @param detectionMeters The maximum distance in meters the robot can be from the coral and still
   *     detect it.
  ...
  private Pose2d getCoralPoseBasic(Pose2d robotPose) {
    ...
    if (useFOV && poseWithinPOV(robotPose, coralPose)) {
      return coralPose;
        ...
        && robotPose.getTranslation().getDistance(coralPose.getTranslation())
            <= detectionDistance) {
      return coralPose;
    }
  ...
  private Pose2d getCoralPoseMapleSim(Pose2d robotPose) {
    ...
    double noise = 0.1;
    return bestCoral.transformBy(
        new Transform2d(randomRange(-noise, noise), randomRange(-noise, noise), Rotation2d.kZero));
  }
  ...
  private boolean poseWithinPOV(Pose2d robotPose, Pose2d coralPose) {
    ...
    Translation2d vertex2 =
        robotTranslation.plus(
            new Translation2d(fovDistance, fovDistance * Math.tan(fovAngle / 2))
                .rotateBy(robotRotation));
    Translation2d vertex3 =
        robotTranslation.plus(
            new Translation2d(fovDistance, -fovDistance * Math.tan(fovAngle / 2))
                .rotateBy(robotRotation));
  ...
  private boolean isPointInTriangle(
      Translation2d pt, Translation2d v1, Translation2d v2, Translation2d v3) {
