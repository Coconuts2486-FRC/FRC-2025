...
public abstract class Field {
  ...
  // Game Manual Page 24 - "...pipes on the same face are 1 ft. 1 in. (~33 cm) apart (center to
  // center)."
    ...
    if (reefAlgaePoses == null) {
      ...
      for (var entry : CURRENT_ALGAE_HEIGHTS.entrySet()) {
        ...
        if (optionalTagPose.isPresent()) {
          ...
          reefAlgaePoses[index] =
              tagPose.transformBy(
                  new Transform3d(-0.15, 0, level == 3 ? 1 : 0.6, Rotation3d.kZero));
        } else {
