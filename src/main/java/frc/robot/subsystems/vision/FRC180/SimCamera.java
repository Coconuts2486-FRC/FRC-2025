...
public class SimCamera {
  ...
  public SimCamera(String name, Transform3d cameraPosition, AprilTagFieldLayout apriltagLayout) {
    ...
    photonPoseEstimator =
        new PhotonPoseEstimator(apriltagLayout, PoseStrategy.LOWEST_AMBIGUITY, cameraPosition);
  ...
  private PoseEstimate toPoseEstimate(
      EstimatedRobotPose estimate, List<PhotonTrackedTarget> targets) {
    ...
    est.avgTagDist =
        targets.stream()
            .mapToDouble(
                t -> t.getBestCameraToTarget().getTranslation().getDistance(Translation3d.kZero))
            .average()
            .orElse(0);
  ...
  private RawFiducial[] toRawFiducials(List<PhotonTrackedTarget> targets) {
    ...
    for (int i = 0; i < targets.size(); i++) {
      ...
      double dist =
          target.getBestCameraToTarget().getTranslation().getDistance(Translation3d.kZero);
